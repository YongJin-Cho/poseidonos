...
def get_qd_and_filename():
    ...
    for i in range(0, int(args.numcpus)):
       command += " --name=test" + str(i) + " --iodepth=" + str(args.qd) + " --filename='trtype=" + str(args.transport) + " adrfam=IPv4 traddr=" + str(args.address) + " trsvcid=1158 subnqn=nqn.2019-04.ibof\:subsystem" + str(i*2+1) + " ns=1'"
    return command
