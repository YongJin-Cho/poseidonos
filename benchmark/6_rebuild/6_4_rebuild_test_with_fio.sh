...
waiting_for_rebuild_complete()
{
	...
	while :
	do
		state=$(${ibof_cli} --json system info | jq '.Response.info.state')
		if [ $state = "\"NORMAL\"" ]; then
			break;
		else
            rebuild_progress=$(${ibof_cli} --json system info | jq '.Response.info.rebuildingProgress')
