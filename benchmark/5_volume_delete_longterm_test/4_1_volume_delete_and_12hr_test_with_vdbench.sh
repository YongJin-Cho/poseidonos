...
do
 ...
 #       sshpass -p ${init1_pw} ssh ${init1_id}@${init1_ip} "echo ${init1_pw} | sudo -S nvme disconnect -n nqn.2019-04.ibof:subsystem${i}"
    else
        echo "init2 disconnect $i"
 #       sshpass -p ${init2_pw} ssh ${init2_id}@${init2_ip} "echo ${init2_pw} | sudo -S nvme disconnect -n nqn.2019-04.ibof:subsystem${i}"
    fi
done
...
do
    ...
    then
      ...
      #  sshpass -p ${init1_pw} ssh ${init1_id}@${init1_ip} "echo ${init1_pw} | sudo -S nvme connect -t tcp -n nqn.2019-04.ibof:subsystem${i} -a ${target_ip_1} -s 1158"
    else
        echo "init2 connect $i"
      #  sshpass -p ${init2_pw} ssh ${init2_id}@${init2_ip} "echo ${init2_pw} | sudo -S nvme connect -t tcp -n nqn.2019-04.ibof:subsystem${i} -a ${target_ip_2} -s 1158"
    fi
done
